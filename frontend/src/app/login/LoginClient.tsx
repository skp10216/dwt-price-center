/**
 * 단가표 통합 관리 시스템 - 로그인 클라이언트 컴포넌트
 *
 * 도메인별 분기 처리:
 * - admin.dwt.price: 프리미엄 다크 + 틸/남색 UI (관리자 권한 필수)
 * - settlement.dwt.price: 프로스티드 블루 UI (경영지원)
...
import { getDomainType, getAfterLoginPath, requiresAdminRole, requiresSettlementRole, type DomainType } from '@/lib/domain';
...
function LoginPageInner({ initialDomainType, isSettlementAdmin = false }: LoginClientProps) {
  ...
  const { domainType, setDomainType } = useDomainStore();

  // SSR-safe: Server Component에서 전달받은 도메인 타입으로 렌더링 분기
  ...
  // 클라이언트에서 도메인 타입 감지 → store 업데이트 (다른 컴포넌트 동기화용)
  useLayoutEffect(() => {
