---
name: 단가표 통합관리 시스템
overview: 본사 판매 단가표를 SSOT(단일 기준 모델) 기반으로 웹에서 통합 관리하고, 거래처별 단가표를 업로드(이미지/엑셀)하여 동일 모델 기준으로 가격을 비교할 수 있는 프리미엄 관리자 시스템을 개발합니다.
todos:
  - id: phase-0-init
    content: "Phase 0: 프로젝트 초기화 - Docker Compose, Next.js/MUI, FastAPI, 인증 시스템"
    status: completed
  - id: phase-1-ssot
    content: "Phase 1: SSOT 모델 관리 - 모델 CRUD, 등급 관리, 등급별 가격, 감사로그"
    status: completed
  - id: phase-2-deductions
    content: "Phase 2: 차감 관리 - 차감 항목/레벨/금액 CRUD, 비활성화 정책"
    status: completed
  - id: phase-3-hq-upload
    content: "Phase 3: 본사 단가표 운영 - 엑셀 업로드, Worker, 검수, 확정/적용"
    status: completed
  - id: phase-4-partner
    content: "Phase 4: 거래처 단가표 운영 - 이미지 OCR, 테이블 변환, SSOT 매칭, 검수"
    status: completed
  - id: phase-5-compare
    content: "Phase 5: 공통 화면 - 본사 단가 리스트, 거래처 비교, 내 리스트 관리"
    status: completed
  - id: phase-6-users
    content: "Phase 6: 사용자 관리 및 고도화 - 계정 관리, 감사로그 UX 고도화"
    status: completed
---
