...
async def _branch_to_response(branch: Branch, db: AsyncSession) -> BranchResponse:
    """Branch → BranchResponse 변환 (partner_count 포함)"""
    count_q = select(func.count(Partner.id)).where(
        Partner.branch_id == branch.id,
        Partner.deleted_at.is_(None),
    )
    result = await db.execute(count_q)
    partner_count = result.scalar() or 0
...
async def create_branch(
    ...
    """지사 생성 (관리자 전용)"""
    # 중복 확인
    result = await db.execute(select(Branch).where(Branch.name == branch_data.name))
...
async def get_branch_impact(
    ...
    partners_q = select(Partner).where(
    ...
    return SuccessResponse(data=BranchImpactResponse(
        ...
        affected_partners=[{"id": str(p.id), "name": p.name} for p in partners],
    ))
...
async def update_branch(
    ...
    if branch_data.name and branch_data.name != branch.name:
        dup = await db.execute(select(Branch).where(Branch.name == branch_data.name))
